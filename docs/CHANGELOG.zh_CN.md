...
- 新增双击 OK 手动息屏及 30 秒闲置自动息屏；蓝牙相遇继续运行，首次按功能键只唤醒屏幕，不改变当前页面。

- 将产品更名为「口袋来信」：更新设备、手机本地编辑页、Wi-Fi 热点、蓝牙显示名及仓库展示；保留 SP v1 名片协议和现有 NVS 命名空间以兼容旧设备与资料。

- 顶部标题和名片昵称固定显示，超长时以省略号结尾；较长的名片正文继续滚动阅读。

- 实装擦肩精修界面：平滑常用中文字形、清晰留白、未读圆点、兴趣标签、可视开关与小信封角色。沿用实体按键导航、离线编辑及现有名片存储。

- 新增擦肩首版固件：深色实体按键界面、离线热点编辑、公开蓝牙名片交换、100 人列表、设置保存及主机验证；真机验收仍待完成。
