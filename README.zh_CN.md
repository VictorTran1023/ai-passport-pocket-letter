...
![口袋来信产品概念封面](assets/images/pocket-letter-hero-v1.png)

# 口袋来信 · Pocket Letter for AI Passport

**随身带一点关于自己的事，留给路上遇见的人。**

为 FoloToy AI Passport 开发的离线社交名片固件。设备靠近时通过蓝牙交换公开名片，手机连接设备自己的 Wi-Fi 热点即可编辑。日常使用不需要账号、云服务器、互联网或配套 App。

[![自动检查](https://github.com/VictorTran1023/ai-passport-pocket-letter/actions/workflows/ci.yml/badge.svg)](https://github.com/VictorTran1023/ai-passport-pocket-letter/actions/workflows/ci.yml)
...
[使用说明](docs/assets/streetpass-guide.zh_CN.md) · [手机编辑页](assets/images/pocket-letter-phone-editor-v1.png) · [构建环境](docs/development/environment-setup.zh_CN.md) · [反馈问题](https://github.com/VictorTran1023/ai-passport-pocket-letter/issues)

## 安静的界面，留给偶然的相遇

![口袋来信实装界面](assets/images/pocket-letter-lvgl-focus-v1.png)
...
![当前固件实际 LVGL 界面](assets/images/pocket-letter-lvgl-preview-v1.png)

全部 12 个实装页面，使用示例资料在电脑渲染。实体按键沿用现有导航；渲染不代表 LCD 显示效果或手机相机扫码验证。[原始视觉提案](assets/images/streetpass-ui-study-v2.png)保留作设计参考。

[查看手机本地编辑页](assets/images/pocket-letter-phone-editor-v1.png) · [查看完整使用说明](docs/assets/streetpass-guide.zh_CN.md)
...
双击 OK 可关闭背光，蓝牙相遇仍在后台运行；首次按上、下或 OK 只唤醒屏幕，不触发页面操作。开启「30 秒自动息屏」后，闲置也会关闭背光，手机编辑期间不自动息屏。独立的硬件电源键没有固件输入事件，因此这版板子无法通过轻按电源键触发此功能。
...
| 验证项目 | 已记录结果 |
| --- | --- |
| Build | 本地 ESP-IDF 5.5.3 构建 PASS；应用 3,133,456 / 3,145,728 字节 |
| Host tests | PASS：协议、导航、存储和仓库检查 |
| UI 与编辑页 | 真实 LVGL 渲染、二维码解码、桌面浏览器编辑页检查通过 |
| Device tests | 息屏固件烧录、40 秒启动观察和兼容蓝牙广播基础检查 PASS；完整真机验收仍待完成 |
| Unverified | 步行相遇、手机自动弹页、耗电、无线功能内存及实体 Recovery |

带日期的产物校验值和验证边界见[验证记录](docs/assets/streetpass-guide.zh_CN.md#息屏功能验证2026-09-23)。上方工作流徽章显示当前远程检查状态，与这里记录的本地结果分别展示。
...
| 路径 | 职责 |
| --- | --- |
| `main/sp_core.*`、`main/sp_nav.*`、`main/sp_screen.*` | 有边界限制的名片格式、蓝牙分片、导航和息屏计时 |
...
| `docs/assets/` | 口袋来信使用说明和设计决定 |

这是在 `main` 上维护口袋来信的独立仓库。保留的上游硬件和示例文档描述原始平台；根目录 README 与口袋来信指南描述本应用。继承的 fork 同步工作流会跳过独立仓库。保留上游历史及署名。

现有 `streetpass` 源文件名、NVS 命名空间和 SP v1 蓝牙载荷保持不变，已保存的名片和旧版兼容设备仍可使用。
